...
def compute_dense(image_filename, detector):
    ...
    #x,y = gray.shape[:2]
    #step = 8	
    #for ix in range(x/step):
	   #for iy in range(y/step):
...
def core(k):
    ...
    #as default k=512
    codebook = compute_codebook("codebook_dense.dat", Train_descriptors, k=k)
    
    
    visual_words = BoW(codebook, Train_descriptors)
    
    # Train a linear SVM claºssifier
    ...
    fo = open('accuracies_dense.txt' ,'a')
    fo.write(out)
    fo.close()
...
if __name__ == "__main__":
    cbook_size_k = np.arange(64,65,256)
        
    for i in cbook_size_k:
